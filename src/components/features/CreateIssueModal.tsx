...
const CreateIssueModal: React.FC<CreateIssueModalProps> = ({ open, onClose, onCreateIssue, project }) => {
  ...
  const handleAddLabel = () => {
    if (newLabel.trim() && !formData.labels.includes(newLabel.trim().toLowerCase())) {
      setFormData(prev => ({
        ...prev,
            ...
            <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
              ...
              <Box sx={{ flex: 1, display: 'flex', alignItems: 'center' }}>
                <Typography variant="body2" sx={{ color: '#5E6C84' }}>
                  Project: <strong>{project.name} ({project.key})</strong>
                </Typography>
              </Box>
            </Box>

...
const matrixTypes = ['Cross-functional Task', 'Department Specific', 'Collaboration', 'Resource Request', 'Report'];

const Matrix: React.FC<MatrixProps> = ({ projectId, projectName, templateType = 'matrix' }) => {
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '50vh' }}>
        <CircularProgress />
        <Typography sx={{ ml: 2 }}>Loading matrix organization workspace...</Typography>
      </Box>
    );
  }
      ...
      <Paper sx={{ m: 3, p: 3, borderRadius: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
          <Box sx={{ flex: 1, minWidth: 200, textAlign: 'center' }}>
            <DualReportIcon sx={{ fontSize: 32, color: 'warning.main', mb: 1 }} />
            ...
            <Box sx={{ flex: 1, overflow: 'auto' }}>
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
                  <Box 
                    sx={{ 
                      ...
                      height: 16, 
                      borderRadius: '50%', 
                      bgcolor: departmentTypes.find(d => d.name === selectedDepartment)?.color 
                    }} 
                  />
                  <Typography variant="h6" fontWeight={600}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  {getDepartmentProgress(selectedDepartment).toFixed(0)}% complete
                </Typography>
                ...
                <Button
                  ...
                  startIcon={<AddIcon />}
                  onClick={() => {
                    setNewTaskData({ ...newTaskData, labels: [selectedDepartment] });
                    setOpenDialog(true);
                  }}
                  sx={{ mb: 2 }}
                >
              ...
              {getTasksByDepartment(selectedDepartment).length > 0 ? (
                getTasksByDepartment(selectedDepartment).map(task => renderTaskCard(task))
              ) : (
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Department(s)"
                  onChange={(e) => setNewTaskData({ ...newTaskData, labels: typeof e.target.value === 'string' ? [e.target.value] : e.target.value })}

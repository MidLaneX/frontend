...
const ChatSidebar: React.FC<ChatSidebarProps> = ({ defaultOpen = false }) => {
  ...
  const [position, setPosition] = useState<Position>({ x: window.innerWidth - 420, y: 100 });
  ...
  const iframeElement = React.useMemo(() => (
    ...
    />
  ), [isDraggableMode]); // Only recreate if draggable mode changes styling
          ...
          <Box sx={STYLES.header} onMouseDown={handleMouseDown}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Tooltip title="Back to Sidebar" arrow placement="top">
                <IconButton onClick={handleDraggableMode} sx={STYLES.iconButton} size="small">
                  <DragIndicatorIcon sx={{ fontSize: 18, transform: 'rotate(90deg)' }} />
                </IconButton>
              </Tooltip>

              <Tooltip title="Minimize" arrow placement="top">
                <IconButton onClick={handleMinimize} sx={STYLES.iconButton} size="small">
                  <MinimizeIcon sx={{ fontSize: 18 }} />
                </IconButton>
              </Tooltip>

              <Tooltip title="Open in New Tab" arrow placement="top">
                <IconButton onClick={handleOpenInNewTab} sx={STYLES.iconButton} size="small">

...
export class ProjectService {
  ...
  static async getProjectById(
    ...
    try {
      ...
      console.log(
        "ProjectService: response data keys:",
  ...
  static async updateProject(
    ...
    try {
      ...
      if (error.response?.status === 403 || error.response?.status === 401) {
        throw new Error(
          "Only administrators can update projects. Please contact your admin.",
        );
      }
  ...
  static async deleteProject(
    ...
    try {
      console.log("ProjectService: Deleting project:", {
        id,
        templateType,
        userId,
      });
      ...
      if (error.response?.status === 403 || error.response?.status === 401) {
        throw new Error(
          "Only administrators can delete projects. Please contact your admin.",
        );
      }
  ...
  static async getAssignedTeam(
    ...
    try {
      ...
      const response = await projectsApi.getAssignedTeam(
        projectId,
        templateType,
      );
